...
public class ShipShooting : ShipBase
{
	...
	void UpdateShooting()
	{
		...
		if( PhotonNetwork.offlineMode == true )
		{
			OnShoot( GetProjectileSpawnPosition()
			        , Quaternion.AngleAxis(Vector2.Angle(RightJoystick.position,Vector2.up), Vector3.up) //ShipVisuals.VisualParent.rotation
					, m_LastProjectileId
					, null );
		}
		else
		{
			...
			PhotonView.RPC( "OnShoot"
							...
							, new object[] { GetProjectileSpawnPosition()
												, RightJoystick.GetRotation() //ShipVisuals.VisualParent.rotation
												, m_LastProjectileId 
										}
							);
		}
	...
	{
		return transform.position; //+ ShipVisuals.VisualParent.up * ( ProjectileSpawnForwardOffset + 100 * PhotonNetwork.GetPing() * 0.001f );
	}
